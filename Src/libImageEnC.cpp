// LibImageEnC.cpp : définit les fonctions exportées pour l'application DLL.
//
#pragma once
#include <iostream>
...
LIBIMAGEENC_API IMAGE allocationImage(int Nblig, int Nbcol)
{
...
LIBIMAGEENC_API void initialisationAleatoireImage(IMAGE img, int ngMin, int ngMax)
{
	int i, j;
...
LIBIMAGEENC_API void liberationImage(IMAGE *img)
{
	if (img->data != NULL) {
		free(img->data);
		img->data = NULL;
...
LIBIMAGEENC_API IMAGE lectureImage(const char *in)
{
	FILE *F = NULL;
	IMAGE img = { 0,0,NULL };
	int dynamique = 0;
...
LIBIMAGEENC_API void sauvegardeImage(IMAGE img, const char *type, const char *out)
{
	FILE *F = NULL;
	...
	if (img.data) /* image non vide */
	{
		...
		if (strcmp(type, "P2") == 0)  /* cas ASCII niveaux de gris */
		{
			...
			fprintf(F, "%s\n", type);
			fprintf(F, "# Created by TSE FISA IPSI 2019-2020\n");
			...
			if (strcmp(type, "P5") == 0)  /* cas brut niveaux de gris */
			{
				...
				fputs("P5\n", F);
				fputs("# Created by TSE FISA IPSI 2019-2020\n", F);
				sprintf(tmp, "%d %d %d\n", img.Nbcol, img.Nblig, 255);
				fputs(tmp, F);
				/* sauvegarde par bloc */
...
LIBIMAGEENC_API IMAGERGB allocationImageRGB(int Nblig, int Nbcol)
{
...
LIBIMAGEENC_API void liberationImageRGB(IMAGERGB *m)
{
	if (m->data != NULL) {
		free(m->data);
		m->data = NULL;
...
LIBIMAGEENC_API IMAGERGB lectureImageRGB(const char *in)
{
	FILE *F = NULL;
...
LIBIMAGEENC_API void sauvegardeImageRGB(IMAGERGB img, const char *type, const char *out)
{
	FILE *F = NULL;
...
LIBIMAGEENC_API IMAGE inverseImage(IMAGE img)
{
...
LIBIMAGEENC_API IMAGE seuillageImage(IMAGE img, int sInf, int sSup)
{
	IMAGE out = { 0,0,NULL,NULL };
	int* lut = NULL;
	int i;
...
LIBIMAGEENC_API IMAGE planImage(IMAGERGB img, int plan)
{
	IMAGE out = { 0,0,NULL,NULL };
	int i;
...
LIBIMAGEENC_API IMAGE luminanceImage(IMAGERGB img, float r, float g, float b)
{
	IMAGE out = { 0,0,NULL,NULL };
	int i;
...
LIBIMAGEENC_API int* histogrammeImage(IMAGE img, int choix)
{
	int* h = NULL;
...
LIBIMAGEENC_API IMAGE expansionImage(IMAGE img, int outMin, int outMax)
{
	IMAGE out = { 0,0,NULL,NULL };
	int* lut = NULL;
	int i;
...
LIBIMAGEENC_API IMAGE egalisationImage(IMAGE img)
{
	IMAGE out = { 0,0,NULL,NULL };
	int* h = histogrammeImage(img, 0);
	int* hC = NULL; 
	int* lut = NULL;
	int i;
...
LIBIMAGEENC_API SIGNATURES statistiquesImage(IMAGE img)
{
	SIGNATURES sig;
	int i;
	double som = 0, som2 = 0;
...
LIBIMAGEENC_API IMAGERGB colorisationImage(IMAGE img, char *table)
{
	FILE *lut;
...
LIBIMAGEENC_API IMAGE seuillageOtsu(IMAGE img)
{
	IMAGE out = { 0, 0, NULL, NULL };
	int* h = histogrammeImage(img, 0);
	int* hC = NULL;
...
/* étiquetage V8 */
LIBIMAGEENC_API IMAGE labelImage(IMAGE img, int *nbComp)
{
	IMAGE out = { 0,0,NULL,NULL };
...
LIBIMAGEENC_API SIGNATURE_COMPOSANTE_CONNEXE* proprietes(IMAGE label, int nbComp, int voisinage)
{
	SIGNATURE_COMPOSANTE_CONNEXE *sig = NULL;
	IMAGE contours = { 0,0,NULL,NULL };
	int i, j;

	sig = (SIGNATURE_COMPOSANTE_CONNEXE*)calloc(nbComp + 1, sizeof(SIGNATURE_COMPOSANTE_CONNEXE));

	for (i = 0; i < label.Nbcol*label.Nblig; i++)
		sig[label.data[i]].surface += 1;

	if (voisinage == 4)
		contours = erosionImage(label, 8);
	else
	...
	for (i = 1; i <= nbComp; i++)
		sig[i].compacite = (sig[i].perimetre*sig[i].perimetre) / (4 * (float)3.1415926 *sig[i].surface);
...
LIBIMAGEENC_API IMAGE erosionImage(IMAGE img, int voisinage)
{
	IMAGE out = { 0,0,NULL,NULL };
	int i, j;
...
LIBIMAGEENC_API IMAGE dilatationImage(IMAGE img, int voisinage)
{
	IMAGE out = { 0,0,NULL,NULL };
	int i, j;
...
LIBIMAGEENC_API IMAGE fermetureImage(IMAGE img, int voisinage)
{
	IMAGE out = { 0,0,NULL,NULL };
...
LIBIMAGEENC_API IMAGE ouvertureImage(IMAGE img, int voisinage)
{
	IMAGE out = { 0,0,NULL,NULL };
	IMAGE inter = { 0,0,NULL,NULL };
