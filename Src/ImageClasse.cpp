...
// germes doiventcorrespondre à une véritable information, notament sur [min max] -> pas de vérif ici
{
	this->m_iHauteur = im.lireHauteur();
	this->m_iLargeur = im.lireLargeur();
...
void CImageClasse::sauvegarde(const std::string& file) {
	...
	if (file.empty())
		nomFichier += this->lireNom(); // force sauvegarde dans répertoire Res (doit exister)
	else
		nomFichier += file;
		...
		if (enregistrementCSV) {
		 std::string fichier = "../Res/" + this->lireNom() + "_SForme.csv";
			...
			if (!f.is_open())
				...
				f << "Objet; CG_i; CG_j; Surface; codeFreeman ; RE_Hi ; RE_Hj ; RE_Bi ; RE_Bj ; perimetre ; eccentricite" << std::endl;
...
// propage couleur moyenne à chaque région, fond restant à 0 ou pas 
CImageCouleur CImageClasse::affichageAxesPrincipaux(const CImageCouleur& img, int R, int G, int B) {
	CImageCouleur out(img);
	out.ecrireNom(this->lireNom() + "_ACP");

	std::vector<SIGNATURE_Forme> tab = this->sigComposantesConnexes(false);

	for (int num = 1; num <= this->lireNbRegions(); num++)
	{
		double ACPx = tab[num].V1y / tab[num].V1x, ACPy = (tab[num].centreGravite_j+0.5) - ACPx*(tab[num].centreGravite_i+0.5);

		for (int x = tab[num].rectEnglob_Hi; x <= tab[num].rectEnglob_Bi; x++)
		{
			int y = (int)(ACPx*x + ACPy);

			if (y >= 0 && (y < this->lireLargeur()) && (this->operator()(x, y) == num))
			{
				out(x, y)[0] = R;
				out(x, y)[1] = G;
				out(x, y)[2] = B;
		...
		for (int y = tab[num].rectEnglob_Hj; y <= tab[num].rectEnglob_Bj; y++)
		{
			int x = (int)((y - ACPy) / ACPx);

			if (x >= 0 && (x < this->lireHauteur()) && (this->operator()(x, y) == num))
			{
				out(x, y)[0] = R;
				out(x, y)[1] = G;
				out(x, y)[2] = B;
		...
		ACPx = tab[num].V2y / tab[num].V2x, ACPy = (tab[num].centreGravite_j + 0.5) - ACPx*(tab[num].centreGravite_i + 0.5);
...
CImageCouleur CImageClasse::affichageAxesPrincipaux(const CImageNdg& img, int R, int G, int B) {
	CImageCouleur out(img);
