...
CImageNdg::CImageNdg(const IMAGE& im) {
	this->m_iHauteur = im.Nblig;
	this->m_iLargeur = im.Nbcol;
	this->m_bBinaire = false;
	this->m_sNom = "inconnuFromC";
	this->m_pucPixel = NULL;
	this->m_pucPalette = new unsigned char[256 * 4];
	...
	this->m_pucPixel = new unsigned char[im.Nblig * im.Nbcol];
	for (int i = 0; i<im.Nblig*im.Nbcol; i++)
		this->m_pucPixel[i] = im.data[i];
}

IMAGE CImageNdg::toCImageNdg() {
	IMAGE mat = { 0,0,NULL,NULL };

	mat.Nblig = this->lireHauteur();
	mat.Nbcol = this->lireLargeur();
	mat.data = (unsigned char*)malloc(mat.Nblig*mat.Nbcol*sizeof(unsigned char));
	for (int i = 0; i < this->lireNbPixels(); i++)
		mat.data[i] = this->operator()(i);

	if (mat.data == NULL)
		return(mat);
	mat.pixel = (unsigned char**)malloc(mat.Nblig*sizeof(unsigned char*));
	if (mat.pixel == NULL) {
		free(mat.data);
		mat.data = NULL;
		return(mat);
	...
	for (int i = 0; i<mat.Nblig; i++)
		mat.pixel[i] = &mat.data[i*mat.Nbcol];
