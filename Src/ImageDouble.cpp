...
std::vector<CImageDouble> CImageDouble::pyramideLaplacienne(int hauteur, int tailleFiltre, double sigma)
{
	std::vector<CImageDouble> burt;
	std::vector<CImageDouble> pyrLap;

	// construction pyramide Burt d'abord

	burt = this->pyramide(hauteur, tailleFiltre, sigma);

	// différence entre niveaux plongés dans la même résolution

	pyrLap.resize(hauteur);
	pyrLap.at(hauteur - 1) = burt.at(hauteur - 1);

	for (int niv = 0; niv < hauteur - 1; niv++)
	{
		CImageDouble inter(burt.at(niv).lireHauteur(), burt.at(niv).lireLargeur());

		inter.ecrireMax(-255); 
		inter.ecrireMin(255);
		inter.ecrireNom(burt.at(niv).lireNom() + std::to_string(niv));

		for (int i = 0; i < burt.at(niv+1).lireHauteur(); i++)
			for (int j = 0; j < burt.at(niv+1).lireLargeur(); j++)
			{
				inter(2 * i, 2 * j) = burt.at(niv + 1)(i, j);
				inter(2 * i + 1, 2 * j) = burt.at(niv + 1)(i, j);
				inter(2 * i, 2 * j + 1) = burt.at(niv + 1)(i, j); 
				inter(2 * i + 1, 2 * j + 1) = burt.at(niv + 1)(i, j);
			}

		for (int i = 0; i < inter.lireNbPixels(); i++)
		{
			inter(i) = burt.at(niv)(i) - inter(i);
			if (inter(i) < inter.lireMin())
				inter.ecrireMin(inter(i));
			if (inter(i) > inter.lireMax())
				inter.ecrireMax(inter(i));
		}
...
CImageDouble CImageDouble::diffoG(int tailleFiltre, double sigma1, double sigma2)
{
	CImageDouble dOG(this->lireHauteur(), this->lireLargeur());
	dOG.ecrireMax(-255);
	dOG.ecrireMin(255);
	dOG.ecrireNom(this->lireNom() + "dOG");

	CImageDouble sig1 = this->filtrage("gaussien", tailleFiltre, sigma1);
	CImageDouble sig2 = this->filtrage("gaussien", tailleFiltre, sigma2);

	for (int i = 0; i < this->lireNbPixels(); i++)
		{
			dOG(i) = sig1(i)-sig2(i);
			if (dOG(i) < dOG.lireMin())
				dOG.ecrireMin(dOG(i));
			if (dOG(i) > dOG.lireMax())
				dOG.ecrireMax(dOG(i));
		}
