...
typedef struct {
	...
	double		V1x; // vecteur principal
	double		V1y;
	double		V2x; // vecteur secondaire
	double		V2y; 
} SIGNATURE_Forme;
...
class CImageClasse {
	...
	///////////////////////////////////////
		...
		// affichage axes principaux -> dans rectangle englobant, 
		// soit couleur unique blanc, soit +1/+2 pour les axes 1/2
		_declspec(dllexport) CImageCouleur affichageAxesPrincipaux(const CImageCouleur& img, int R = 255, int G = 0, int B=0);
		_declspec(dllexport) CImageCouleur affichageAxesPrincipaux(const CImageNdg& img, int R = 255, int G = 0, int B = 0);
